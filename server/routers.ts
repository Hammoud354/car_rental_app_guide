...
export const appRouter = router({
  ...
  fleet: router({
    ...
    create: publicProcedure
      .input(z.object({
        ...
        purchaseType: z.enum(["Cash", "Installments"]).optional(),
        downPayment: z.string().optional(),
        interestRate: z.string().optional(),
        monthlyInstallmentAmount: z.string().optional(),
        numberOfInstallments: z.number().int().optional(),
        remainingBalance: z.string().optional(),
        sellerName: z.string().max(200).optional(),
        vehicleRegistrationNumber: z.string().max(100).optional(),
  ...
  contracts: router({
    ...
    create: protectedProcedure
      .input(z.object({
        ...
        clientMotherFullName: z.string().max(200).optional(),
        clientFatherFullName: z.string().max(200).optional(),
        clientDateOfBirth: z.string().optional(),
        clientPlaceOfBirth: z.string().max(200).optional(),
        clientPassportNumber: z.string().max(100).optional(),
        clientRegistrationNumber: z.string().max(100).optional(),
        clientPlaceOfRegistration: z.string().max(200).optional(),
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const contract = await db.createRentalContract({
          ...
          clientMotherFullName: input.clientMotherFullName || null,
          clientFatherFullName: input.clientFatherFullName || null,
          clientDateOfBirth: input.clientDateOfBirth ? new Date(input.clientDateOfBirth) : null,
          clientPlaceOfBirth: input.clientPlaceOfBirth || null,
          clientPassport: input.clientPassportNumber || null,
          clientRegistrationNumber: input.clientRegistrationNumber || null,
          clientPlaceOfRegistration: input.clientPlaceOfRegistration || null,
  ...
  admin: router({
    ...
    createUser: superAdminProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        // Auto-assign Starter plan to admin-created users
        try {
          await db.initializeSubscriptionTiers();
          const existingSub = await db.getUserSubscription(newUser.id);
          if (!existingSub) {
            await db.createUserSubscription(newUser.id, 1, "Auto-assigned Starter plan on admin user creation");
          }
        } catch (error) {
          console.error('Failed to auto-assign subscription for admin-created user:', error);
        }
        ...
        for (const client of input.clients) {
          try {
            ...
            const created = await db.createClient({
              ...
              email: client.email,
              phone: client.phone,
              driverLicenseNumber: client.drivingLicenseNumber || 'N/A',
  ...
  privacy: router({
    ...
    // Super admin: read privacy settings for any user
    adminGetUserPrivacy: superAdminProcedure
