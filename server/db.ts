...
export async function createMaintenanceRecord(record: InsertMaintenanceRecord & { markInMaintenance?: boolean }) {
  ...
  if (shouldMarkMaintenance && record.vehicleId && record.userId) {
    const vw = await ownerWhere(vehicles, vehicles.id, record.vehicleId, vehicles.userId, record.userId);
...
export async function getRentalContractsByStatus(userId: number, status?: "active" | "completed" | "overdue", filterUserId?: number) {
  ...
  return effectiveFilter != null
    ? await db.select().from(rentalContracts).where(and(eq(rentalContracts.userId, effectiveFilter), eq(rentalContracts.status, status)))
...
export async function getClientByLicenseNumber(licenseNumber: string, userId: number): Promise<Client | undefined> {
  ...
  const admin = await isSuperAdmin(userId);
  const conditions: any[] = [eq(clients.drivingLicenseNumber, licenseNumber)];
...
export async function renewVehicleInsurance(
  ...
  const admin = await isSuperAdmin(userId);
  const ipConditions: any[] = [eq(insurancePolicies.vehicleId, vehicleId), eq(insurancePolicies.status, 'active')];
...
export async function getActiveInsurancePolicy(vehicleId: number, userId: number) {
  ...
  const admin = await isSuperAdmin(userId);
  const conditions: any[] = [eq(insurancePolicies.vehicleId, vehicleId), eq(insurancePolicies.status, 'active')];
  if (!admin) conditions.push(eq(insurancePolicies.userId, userId));
