...
export function ExpiringDocumentsModal({ isOpen, onOpenChange }: ExpiringDocumentsModalProps) {
  ...
  const getUrgencyBadge = (daysRemaining: number) => {
    if (daysRemaining <= 0) return { label: "Expired", color: "bg-red-600 text-white" };
  ...
  const getDocIcon = (documentType: string) => {
    if (documentType === "Insurance Policy") return <Shield className="h-5 w-5 text-blue-500" />;
    if (documentType === "Vehicle Registration") return <Car className="h-5 w-5 text-purple-500" />;
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Expired and documents expiring within the next 30 days. Click on any document to take action.
          </DialogDescription>
        </DialogHeader>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        ...
                        <div>
                          ...
                          <p className={`font-medium ${
                            doc.daysRemaining <= 0 ? "text-red-700" :
